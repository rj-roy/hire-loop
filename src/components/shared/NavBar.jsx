                            ...
                            {session ? (
                                <div className="flex items-center space-x-4">
                                    <span className="text-gray-800 dark:text-gray-300">
                                        Welcome! <span className="font-semibold">{session.user?.name || session.user?.email || 'User'}</span>
                                    </span>
                                </div>
                            ) : (
                                <>
                    ...
                    <div className="md:hidden mt-4 pb-4 border-t border-gray-800 pt-4">
                        <div className="flex flex-col space-y-4">
                            ...
                            <div className="border-t border-gray-800 pt-4 mt-4 flex flex-col space-y-3">
                                {session ? (
                                    <div className="text-gray-800 dark:text-gray-300 px-2 text-center">
                                        Welcome! <span className="font-semibold">{session.user?.name || session.user?.email || 'User'}</span>
                                    </div>
                                ) : (
                                    <>
                                        ...
                                        <Link
                                            href="/get-started"
                                            className="bg-linear-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-6 py-2.5 rounded-lg font-medium transition-all duration-200 shadow-lg hover:shadow-xl text-center mx-2"
                                            onClick={() => setIsMobileMenuOpen(false)}
                                        >
                                            Get Started
                                        </Link>
